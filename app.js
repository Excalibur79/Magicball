...
io.on("connection", (socket) => {
  ...
  socket.on("join", async (data, cb) => {
    ...
    var akiclone = akiarray.find(
      (ob) => JSON.stringify(ob) === JSON.stringify(aki)
    );
  ...
  socket.on("answered", async (data, cb) => {
    ...
    if (aki.aki.progress >= 70 || aki.aki.currentStep >= 78) {
      await aki.aki.win();
      io.to(aki.aki.session).emit("Foundmatches", {
        matches: aki.aki.answers,
        guessCount: aki.aki.guessCount,
      });
      cb();
    } else {
